...
class UnsubAckTest extends \PHPUnit\Framework\TestCase {
  ...
  protected function setUp() {
    ...
    $this->frame->packetType = \Mqtt\Protocol\IPacketType::UNSUBACK;
    $this->frame->flags = clone $this->___container->get(\Mqtt\Protocol\Binary\Data\Uint8::class);
    $this->frame->flags->set(\Mqtt\Protocol\IPacketReservedBits::FLAGS_UNSUBACK);
  ...
  public function testInitialCleanState() {
    ...
    $this->assertEquals(\Mqtt\Protocol\IPacketType::UNSUBACK, $object->get()->getType());
  }
  ...
  public function testProperlyDecodingReturnsUnsubAckPacketEntity() {
    $this->object->decode($this->frame);
    $this->assertInstanceOf(\Mqtt\Protocol\Entity\Packet\UnsubAck::class, $this->object->get());
  }
